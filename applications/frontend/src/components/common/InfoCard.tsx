...
export const InfoCard = ({ icon: Icon, label, value, variant = 'outlined', tint = 'primary' }: InfoCardProps) => {
    ...
    const content = (
        <Stack direction="row" spacing={1.2} alignItems="start">
            <Box sx={{ width: 38, height: 38, borderRadius: 3, bgcolor: iconBg, color: iconColor, display: 'grid', placeItems: 'center', flex: 'none' }}>
                <Icon size={16} />
            </Box>
            <Box sx={{ minWidth: 0 }}>
    ...
    if (variant === 'border') {
        return (
            <Box sx={{ border: '1px solid', borderColor: 'border', borderRadius: 4, p: 1.8 }}>
                {content}
            </Box>
        );
    }
