...
function ParticipantTokensRow({
    participantId,
    campaignId,
    colSpan,
}: { participantId: number; campaignId: number; colSpan: number }) {
...
function AdminCampaignDetailRoute() {
    ...
    const handleInvite = async () => {
        ...
        setSnack(
            `${result.created} invitation${result.created !== 1 ? 's' : ''} envoyée${result.created !== 1 ? 's' : ''}`
        );
    };
    ...
    const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        setSnack(
            `${result.created} créé${result.created !== 1 ? 's' : ''}, ${result.invited} invité${result.invited !== 1 ? 's' : ''}`
        );
    ...
    return (
        <Stack spacing={3}>
            ...
            {inviteParticipants.isError && (
                <Alert severity="error" onClose={() => inviteParticipants.reset()}>
                    Erreur lors de l'envoi des invitations.
                </Alert>
            )}
            {importParticipants.isError && (
                <Alert severity="error" onClose={() => importParticipants.reset()}>
                    Erreur lors de l'import : {importParticipants.error?.message}
                </Alert>
            )}
                        ...
                        <CardContent sx={{ p: 2.5 }}>
                            ...
                            <Stack direction="row" spacing={1.2} sx={{ mt: 2.5, flexWrap: 'wrap' }}>
                                <Chip
                                    ...
                                    label={`Créée le ${campaign.createdAt ? new Date(campaign.createdAt).toLocaleDateString('fr-FR') : '\u2013'}`}
                                    sx={{ borderRadius: 99 }}
                                />
                                {campaign.startsAt && (
                                    <Chip
                            ...
                            <Stack spacing={1.2} sx={{ mt: 2 }}>
                                ...
                                {(campaign.status === 'draft' ||
                                    ...
                                    <Button
                                        ...
                                        disabled={updateStatus.isPending}
                                        onClick={() =>
                                            updateStatus.mutate({ campaignId: campaign.id, status: 'archived' })
                                        }
