  ...
  if (isLoading) {
    return (
      <Card variant="outlined">
        <CardContent sx={{ p: 3 }}>
          <Typography variant="h6" fontWeight={800} color="text.primary">Chargement de votre coach</Typography>
          ...
          <Stack spacing={2.5} direction={{ xs: "column", lg: "row" }} justifyContent="space-between" alignItems={{ xs: "start", lg: "start" }}>
            ...
            <Card variant="outlined" sx={{ width: { xs: "100%", sm: 340 } }}>
              <CardContent sx={{ p: 2, display: "flex", gap: 1.5, alignItems: "center" }}>
                ...
                <Box sx={{ minWidth: 0 }}>
                  <Typography fontWeight={800} color="text.primary">{coachView.name}</Typography>
                  <Typography variant="body2" color="text.secondary">{coachView.title} · {coachView.company}</Typography>
                </Box>
              </CardContent>
            </Card>
          </Stack>
      ...
      <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", md: "repeat(3, minmax(0, 1fr))" }, gap: 2 }}>
        <Card variant="outlined">
          <CardContent sx={{ p: 2.3 }}>
            ...
            <Typography variant="h5" fontWeight={800} color="text.primary" sx={{ mt: 0.5 }}>{coachView.status}</Typography>
          </CardContent>
        </Card>
        <Card variant="outlined">
          <CardContent sx={{ p: 2.3 }}>
            <Typography variant="body2" color="text.secondary">Campagne</Typography>
            <Typography variant="h5" fontWeight={800} color="text.primary" sx={{ mt: 0.5 }}>{coachView.campaignName}</Typography>
          </CardContent>
        </Card>
        <Card variant="outlined">
          <CardContent sx={{ p: 2.3 }}>
            <Typography variant="body2" color="text.secondary">Questionnaire</Typography>
            <Typography variant="h5" fontWeight={800} color="text.primary" sx={{ mt: 0.5 }}>{coachView.questionnaire}</Typography>
          </CardContent>
        </Card>
      </Box>
